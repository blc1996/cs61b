...
public class RunIterator implements Iterator {
  ...
  // the encoding.
  RunIterator(DList2 Pix) {
    // Your solution here.  You may add parameters to the method signature.
      values = new int[4];
      List = Pix;
      values[0] = Pix.head.next.itemNum;
      values[1] = Pix.head.next.redVal;
      values[2] = Pix.head.next.greenVal;
      values[3] = Pix.head.next.blueVal;
      totalNum = Pix.size;
      currentNum = 0;
  }
  ...
  public int[] next() {
    ...
    // Replace the following line with your solution.
      ...
      currentNode = List.nth(currentNum);
      values[0] = currentNode.itemNum;
      values[1] = currentNode.redVal;
      values[2] = currentNode.greenVal;
      values[3] = currentNode.blueVal;
    return values;
