...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
      this.width = width;
      this.height = height;
      Pix = new DList2(width * height, 0, 0, 0);
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    //  DListNode2 buffNode;
      ...
      this.height = height;
      Pix = new DList2();
      for(i = 0; i < runLengths.length; i++){
          Pix.insertEnd(runLengths[i], red[i], green[i], blue[i]);
      }
  }
  ...
  public PixImage toPixImage() {
    // Replace the following line with your solution.
      ...
      temp = iter.next();
      for(i = 0; i < temp[0]; i++){
          Pic.setPixel(x, y, (short)temp[1], (short)temp[2], (short)temp[3]);
          x++;
          count++;
          if(count >= width){
              x=0;
  ...
  public String toString() {
    // Replace the following line with your solution.
      String result = "[  ";
      DListNode2 current = Pix.head.next;
      while (current != Pix.head) {
          result = result + current.itemNum + " " + current.redVal + " " + current.greenVal + " " + current.blueVal + "|";
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    // at the end.
      ...
      int x = 0, y = 0, count = 0, i;
      Pix = new DList2();
//      for(i = 0; i< (width * height); i++){
//          int temp = y + x * height;
//          red[temp] = image.getRed(x, y);
//          green[temp] = image.getGreen(x, y);
//          blue[temp] = image.getBlue(x, y);
//          y++;
//          count++;
//          if(count >= height){
//              y = 0;
...
//      }
      ...
      int redTemp = red[0], greenTemp = green[0], blueTemp = blue[0];
      //System.out.print(String.valueOf(image.getRed(0, 2)));
      count = 1;
      for(i = 1; i < (width * height); i++){
          if(redTemp == red[i] && greenTemp == green[i] && blueTemp == blue[i]){
              count++;
          }else{
              Pix.insertEnd(count, redTemp, greenTemp, blueTemp);
              count = 1;
              redTemp = red[i];
              greenTemp = green[i];
              blueTemp = blue[i];
  ...
  public void check() {
    // Your solution here.
      DListNode2 currentNode = Pix.head.next;
      while(currentNode.next != Pix.head){
          if(currentNode.redVal == currentNode.next.redVal && currentNode.greenVal == currentNode.next.greenVal && currentNode.blueVal == currentNode.next.blueVal){
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    ...
    //   at the end.
      ...
      temp = this.toPixImage();
      temp.setPixel(x, y, red, green, blue);
      RunLengthEncoding replace = new RunLengthEncoding(temp);
      this.width = replace.width;
      this.height = replace.height;
      this.Pix = replace.Pix;
    check();
  }
